          ...
          {attributes && Object.keys(attributes).map((attribute) => {
            return (
              <tr key={`${RESOURCE.singular}-attr-${attribute}`}>
                <td>{attribute}</td>
                <td>
                  <Attribute name={attribute} metadata={data?.included} value={attributes[attribute]}/>
                </td>
              </tr>
            )
          })}
  ...
  return (
    <div id={RESOURCE.plural}>
      <Breadcrumb>
        ...
        <Breadcrumb.Item active>{isLoading ? <Spinner animation="border" size="sm" variant="primary" className="align-self-center" /> : attributes?.name}</Breadcrumb.Item>
      </Breadcrumb>
      <h2>Planet: {isLoading ? <Spinner animation="border" size="sm" variant="primary" className="align-self-center" /> : attributes?.name}</h2>
